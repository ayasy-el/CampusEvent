...
<a href="{{ $href }}"
    {{ $attributes->class([
        ...
        'text-xs md:text-sm text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300 px-0 py-0 rounded-none shadow-none' => $variant === 'link-sm',
        'px-4 py-1.5 rounded-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-black dark:text-white hover:bg-slate-50 dark:hover:bg-slate-600' => $variant === 'ghost-white',
        'px-4 py-2 rounded-full bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-xs font-semibold hover:bg-slate-800 dark:hover:bg-slate-200' => $variant === 'dark-sm',
