...
<aside class="space-y-4 md:space-y-5 md:sticky md:top-20 h-max">
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm shadow-slate-200/60 dark:shadow-slate-900/60">
        <img src="{{ $event['image'] }}" alt="{{ $event['title'] ?? 'Poster event' }}" class="w-full object-contain">
    </div>
    @endif

    <section class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md shadow-slate-200/80 dark:shadow-slate-900/80 p-4 md:p-5 space-y-3">
        <div class="flex items-center justify-between">
            ...
            <span
                class="px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800 text-[11px] text-emerald-700 dark:text-emerald-400 font-medium">
        ...
        <div class="border-t border-slate-100 dark:border-slate-700 pt-3 mt-2 space-y-2">
            <div class="flex items-center justify-between text-xs text-slate-700 dark:text-slate-300">
                <span>Harga tiket</span>
                <span class="font-semibold text-slate-900 dark:text-white">{{ $event['price_display'] ?? 'Gratis' }}</span>
            </div>
    ...
    <section class="bg-white/90 dark:bg-slate-800/90 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-3.5 md:p-4 space-y-2">
        <p class="text-xs font-semibold text-slate-900 dark:text-white">
            Informasi &amp; Kontak
        </p>
        ...
        <ul class="text-[11px] text-slate-600 dark:text-slate-400 space-y-1">
            ...
            <li>Email: <span class="font-medium text-slate-800 dark:text-slate-200">{{ $event['contact_email'] }}</span></li>
            @endif
            @if (!empty($event['contact_phone']))
            <li>Telepon: <span class="font-medium text-slate-800 dark:text-slate-200">{{ $event['contact_phone'] }}</span></li>
            @endif
            @if (empty($event['contact_email']) && empty($event['contact_phone']))
            <li class="text-slate-400 dark:text-slate-500">Kontak panitia belum tersedia.</li>
            @endif
        </ul>
    </section>
</aside>