...
<article @class([ 'bg-white/95 h-full border border-slate-100 shadow-sm hover:shadow-md transition group' , 'rounded-2xl p-4'=> $variant === 'list',
    ...
    data-date="{{ ($event['start_date'] ?? $event['date'])->format('Y-m-d') }}" data-index="{{ $index }}"
    ...
    <div class="flex gap-4">
        ...
        <div class="relative hidden md:flex w-25 h-25 rounded-2xl overflow-hidden bg-slate-200 flex-shrink-0">
            ...
            <div class="absolute top-1 left-1 px-1.5 py-1 rounded-2xl bg-slate-900/90 text-white text-center min-w-[44px]">
                ...
                <div class="flex items-center justify-center gap-0.5">
                    <span class="text-[10px] font-bold leading-none">{{ $startDate->format('d') }}</span>
                    <span class="text-[8px] text-slate-400">-</span>
                    <span class="text-[10px] font-bold leading-none">{{ $endDate->format('d') }}</span>
                </div>
                <span class="text-[8px] text-slate-300 block mt-0.5">
                ...
                @else
                <span class="text-[9px] uppercase tracking-wide text-slate-200 block">{{ $startDate->translatedFormat('D') }}</span>
                <span class="text-xs font-bold leading-none block">{{ $startDate->format('d') }}</span>
                <span class="text-[9px] text-slate-300 block mt-0.5">{{ $startDate->translatedFormat('M') }}</span>
                @endif
            </div>
        </div>
        @else
        <div class="hidden md:flex flex-col items-center justify-center px-4 py-2 bg-slate-900 text-white rounded-xl min-w-[68px]">
            ...
            <div class="flex items-center justify-center gap-0.5">
                <span class="text-base font-bold">{{ $startDate->format('d') }}</span>
                <span class="text-xs text-slate-400">-</span>
                <span class="text-base font-bold">{{ $endDate->format('d') }}</span>
            </div>
            ...
            @else
            <span class="text-[11px] uppercase text-slate-300">{{ $startDate->translatedFormat('D') }}</span>
            <span class="text-xl font-bold">{{ $startDate->format('d') }}</span>
            <span class="text-[11px] text-slate-300">{{ $startDate->translatedFormat('M') }}</span>
            @endif
        </div>
        @endif
        ...
        <!-- CTA Desktop -->
        @if ($show_actions)
        <div @class([ 'hidden md:flex'=> $cardStatus === 'open',
    ...
    <div class="relative h-32">
        ...
        <div class="absolute top-2 left-2 px-2 py-1.5 bg-slate-900/90 text-white rounded-2xl text-center">
            ...
            <div class="flex items-center justify-center gap-0.5">
                <span class="text-sm font-bold">{{ $startDate->format('d') }}</span>
                <span class="text-[10px] text-slate-400">-</span>
                <span class="text-sm font-bold">{{ $endDate->format('d') }}</span>
            </div>
            <span class="text-[8px] text-slate-300 block mt-0.5">
                @if($startDate->format('M') === $endDate->format('M'))
                {{ $startDate->translatedFormat('M') }}
            ...
            @else
            <span class="text-[10px] uppercase text-slate-200 block">{{ $startDate->translatedFormat('D') }}</span>
            <span class="text-base font-bold block">{{ $startDate->format('d') }}</span>
            <span class="text-[10px] text-slate-300 block mt-0.5">{{ $startDate->translatedFormat('M') }}</span>
            @endif
        </div>
        @else
        <div class="flex flex-col items-center justify-center bg-slate-900 text-white rounded-xl min-h-full min-w-full">
            @if($isMultiDay)
            <span class="text-sm uppercase text-slate-300">{{ $startDate->translatedFormat('D') }} - {{ $endDate->translatedFormat('D') }}</span>
            <div class="flex items-center justify-center gap-1">
                <span class="text-2xl font-bold">{{ $startDate->format('d') }}</span>
                <span class="text-base text-slate-400">-</span>
                <span class="text-2xl font-bold">{{ $endDate->format('d') }}</span>
            </div>
            ...
            @else
            <span class="text-[20px] uppercase text-slate-300">{{ $startDate->translatedFormat('D') }}</span>
            <span class="text-[30px] font-bold">{{ $startDate->format('d') }}</span>
            <span class="text-[20px] text-slate-300">{{ $startDate->translatedFormat('M') }}</span>
            @endif
        </div>
        @endif
