...
<section class="space-y-3 md:space-y-4">
    ...
    <div class="mt-4 flex items-center justify-between text-[11px] md:text-xs text-slate-500 dark:text-slate-400">
        ...
        <div class="inline-flex items-center gap-1">
            <a href="{{ $events->previousPageUrl() ?? '#' }}" @class([ 'px-3 py-1 rounded-full border border-slate-200 dark:border-slate-600 transition' , 'bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200'=> $events->previousPageUrl(),
            ...
            <a href="{{ $events->url($page) }}" @class([ 'px-3 py-1 rounded-full' , 'bg-slate-900 dark:bg-sky-600 text-white font-semibold'=> $page === $currentPage,
            ...
            @if ($currentPage + 1 <= $lastPage)
                ...
                <a href="{{ $events->nextPageUrl() ?? '#' }}" @class([ 'px-3 py-1 rounded-full border border-slate-200 dark:border-slate-600 transition' , 'bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200'=> $events->nextPageUrl(),
